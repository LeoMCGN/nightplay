# L'Imposteur Online — Plan d'implémentation

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ajouter une version multijoueur de L'Imposteur où chaque joueur voit son mot secret sur son propre device via Supabase Realtime, avec vote simultané et synchronisation temps réel.

**Architecture:** Nouveau composant `ImposteurOnline.jsx` indépendant (l'existant `Imposteur.jsx` reste intact). 3 tables Supabase préfixées `imp_`. Le host distribue les mots et pilote les transitions d'état. Les joueurs votent simultanément, le host calcule les résultats quand tous ont voté.

**Tech Stack:** React 19, Supabase JS v2, Framer Motion, Tailwind CSS, `src/data/imposteur.json` (paires existantes)

> **Note TDD :** Aucune infrastructure de tests dans ce projet. Vérifications manuelles dans le navigateur.
...
|---|---|---|
| `supabase/schema-imposteur.sql` | Créer | SQL à exécuter dans le dashboard Supabase |
| `src/pages/games/ImposteurOnline.jsx` | Créer | Composant principal toutes phases |
| `src/data/games.js` | Modifier | Ajouter l'entrée imposteur-online |
| `src/App.jsx` | Modifier | Ajouter la route `/play/imposteur-online` |
...
**Files:**
- Create: `supabase/schema-imposteur.sql`

- [ ] **Step 1 : Créer le fichier SQL**

Créer `supabase/schema-imposteur.sql` :
...
Aller dans **Supabase → SQL Editor → New query**, coller le contenu, cliquer **Run**. Vérifier que les 3 tables `imp_rooms`, `imp_players`, `imp_rounds` apparaissent dans Table Editor.

- [ ] **Step 3 : Commit**

```bash
git add supabase/schema-imposteur.sql
git commit -m "feat: add imposteur online supabase schema"
```

---

## Task 2 : Enregistrer le jeu + squelette du composant

**Files:**
- Modify: `src/data/games.js`
- Modify: `src/App.jsx`
- Create: `src/pages/games/ImposteurOnline.jsx`

- [ ] **Step 1 : Ajouter l'entrée dans games.js**

Lire `src/data/games.js`, puis ajouter après l'entrée `imposteur` :
...
Lire `src/App.jsx`, puis ajouter l'import et la route :

```jsx
import ImposteurOnline from './pages/games/ImposteurOnline'
```

```jsx
<Route path="/play/imposteur-online" element={<ImposteurOnline />} />
```

- [ ] **Step 3 : Créer le squelette ImposteurOnline.jsx**

Créer `src/pages/games/ImposteurOnline.jsx` :
...
Lancer `npm run dev`. La carte "L'Imposteur Online" doit apparaître sur la Home. Cliquer dessus → page "ImposteurOnline — phase: entry".

- [ ] **Step 5 : Commit**

```bash
git add src/data/games.js src/App.jsx src/pages/games/ImposteurOnline.jsx
git commit -m "feat: register imposteur online + skeleton component"
```

---

## Task 3 : Phase entry + lobby (avec Realtime)

**Files:**
- Modify: `src/pages/games/ImposteurOnline.jsx`

- [ ] **Step 1 : Remplacer le return du squelette par le composant complet avec entry + lobby**

Lire `src/pages/games/ImposteurOnline.jsx` en entier, puis remplacer tout ce qui suit `const isLastRound = ...` par :
...
Ouvrir deux onglets. Onglet 1 : créer une partie → lobby avec code violet. Onglet 2 : rejoindre avec le code → les deux onglets voient les deux joueurs en temps réel.

- [ ] **Step 3 : Commit**

```bash
git add src/pages/games/ImposteurOnline.jsx
git commit -m "feat: imposteur online entry and lobby phases with realtime"
```

---

## Task 4 : Phases distribution, discussion, vote

**Files:**
- Modify: `src/pages/games/ImposteurOnline.jsx`

- [ ] **Step 1 : Ajouter les 3 phases dans le JSX**

Lire le fichier. Dans `<AnimatePresence>`, après la fermeture de `{phase === 'lobby' && (...)}` et AVANT `</AnimatePresence>`, ajouter :
...
- [ ] **Step 2 : Vérifier manuellement avec 3 onglets**

- Créer une partie (onglet 1), rejoindre avec 2 autres onglets
- Lancer → distribution : chaque onglet voit son propre mot
- Cliquer "Prêt" sur les 3 → discussion démarre automatiquement
- Host clique "Passer au vote" → les 3 onglets passent en vote
- Voter sur chaque onglet → compteur "X/3 ont voté"

- [ ] **Step 3 : Commit**

```bash
git add src/pages/games/ImposteurOnline.jsx
git commit -m "feat: imposteur online distribution, discussion and vote phases"
```

---

## Task 5 : Phases revealing, result, finished

**Files:**
- Modify: `src/pages/games/ImposteurOnline.jsx`

- [ ] **Step 1 : Ajouter les 3 dernières phases dans le JSX**

Lire le fichier. Dans `<AnimatePresence>`, après la fermeture de `{phase === 'vote' && (...)}` et AVANT `</AnimatePresence>`, ajouter :
...
- [ ] **Step 2 : Vérifier le flux complet**

Jouer une partie complète à 3 onglets :
1. Lobby → 3 joueurs → lancer
2. Distribution → chaque onglet voit son mot → Prêt
3. Discussion → timer → vote
4. Tous votent → revealing 2.5s → result avec mots révélés + scores
5. Manche suivante ou podium final

- [ ] **Step 3 : Commit**

```bash
git add src/pages/games/ImposteurOnline.jsx
git commit -m "feat: imposteur online revealing, result and finished phases"
```
...
Tester une partie complète sur deux téléphones physiques.
...
| Requirement | Task |
|---|---|
| Tables `imp_rooms`, `imp_players`, `imp_rounds` + RLS | Task 1 |
| Entry : créer/rejoindre avec code | Task 3 |
| Lobby temps réel, config host (timer, manches) | Task 3 |
| Min 3 joueurs pour lancer | Task 3 |
| Distribution : chaque joueur voit son mot sur son device | Task 4 |
| Badge imposteur orange | Task 4 |
| Bouton Prêt → transition auto vers discussion | Task 4 |
| Timer discussion synchronisé | Task 3 |
| Host peut passer au vote avant expiration | Task 4 |
| Vote simultané, compteur X/Y | Task 4 |
| Calcul résultats : +1 innocents / +2 imposteur | Task 3 |
| Révélation 2.5s | Task 5 |
| Result : mots révélés, votes, scores | Task 5 |
| Host : manche suivante ou podium | Task 5 |
| Podium animé phase finished | Task 5 |
| Deploy | Task 6 |
